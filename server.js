...
// Shows candles, FVG, OB, S/R levels, TP, SL
// -------------------------------------------------------
function generateChartSVG(price, symbol) {
  ...
  // Deterministic seed from price+symbol for unique shapes
  ...
  for (let i = 0; i < numC; i++) {
    ...
    const wickUp = sr(i + 20) * range * 0.14;
    const wickDown = sr(i + 30) * range * 0.14;
    candles.push({
      open,
      close,
  ...
  if (toY(resistance) > pt && toY(resistance) < pt + ch) {
    svg += `<line x1="${pl}" y1="${toY(resistance)}" x2="${w-pr}" y2="${toY(resistance)}" stroke="rgba(255,45,85,0.35)" stroke-width="1" stroke-dasharray="6,4"/>`;
    svg += `<text x="${pl+3}" y="${toY(resistance)-2}" font-family="JetBrains Mono,monospace" font-size="7" fill="rgba(255,45,85,0.7)">R1</text>`;
  }

  // Support level
  if (toY(support) > pt && toY(support) < pt + ch) {
    svg += `<line x1="${pl}" y1="${toY(support)}" x2="${w-pr}" y2="${toY(support)}" stroke="rgba(0,232,122,0.35)" stroke-width="1" stroke-dasharray="6,4"/>`;
    svg += `<text x="${pl+3}" y="${toY(support)+9}" font-family="JetBrains Mono,monospace" font-size="7" fill="rgba(0,232,122,0.7)">S1</text>`;
  }
  ...
  candles.forEach((c, i) => {
    ...
    svg += `<rect x="${x-cndW/2}" y="${bt}" width="${cndW}" height="${bh}" fill="${col}" opacity="${bull?0.88:0.82}"/>`;
  });

  // TP line
  const tpY = toY(target);
  if (tpY > pt - 5 && tpY < pt + ch + 5) {
    svg += `<line x1="${pl}" y1="${tpY}" x2="${w-pr}" y2="${tpY}" stroke="#00e87a" stroke-width="1.2" stroke-dasharray="5,3" opacity="0.95"/>`;
    svg += `<text x="${w-pr+4}" y="${tpY+3}" font-family="JetBrains Mono,monospace" font-size="8" fill="#00e87a">TP</text>`;
  }

  // SL line
  const slY = toY(stop);
  if (slY > pt - 5 && slY < pt + ch + 5) {
    svg += `<line x1="${pl}" y1="${slY}" x2="${w-pr}" y2="${slY}" stroke="#ff2d55" stroke-width="1.2" stroke-dasharray="5,3" opacity="0.95"/>`;
    svg += `<text x="${w-pr+4}" y="${slY+3}" font-family="JetBrains Mono,monospace" font-size="8" fill="#ff2d55">SL</text>`;
  }
  ...
  // Entry arrow
  const lx = toX(numC-1);
  svg += `<polygon points="${lx+5},${alertY} ${lx+13},${alertY-5} ${lx+13},${alertY+5}" fill="${prof.color}" opacity="0.95"/>`;

  // Symbol watermark
  svg += `<text x="${w-pr-4}" y="${pt+ch}" font-family="JetBrains Mono,monospace" font-size="9" fill="rgba(255,255,255,0.08)" text-anchor="end">${symbol||""}</text>`;
...
- Target: a SIGNIFICANT take profit level — aim for at least 2-3x the stop distance away, targeting the next major liquidity pool or swing high/low. Give the specific price.
- Stop: specific stop loss price just beyond the structure
- Structure: did price break structure bullish or bearish here

Rules: no asterisks, no markdown, no bold. Real prices near $${price}. 4-5 sentences max. Write like a sharp prop trader texting his buddy. Direct and specific. Big targets — don't be conservative.
